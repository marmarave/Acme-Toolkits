...
@Service
public class InventorItemQuantityToolsListService implements AbstractListService<Inventor, ItemQuantity>{
	...
	public Collection<ItemQuantity> findMany(final Request<ItemQuantity> request) {
		...
		final Collection<ItemQuantity> result = itemQuantities.stream().filter(x->x.getItem().getType().equals(ItemType.TOOL)).collect(Collectors.toCollection(HashSet::new));
		return result;
